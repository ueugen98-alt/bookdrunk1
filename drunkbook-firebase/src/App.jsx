...
useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    getDoc(convRef).then(s=>{if(s.exists()){const rb=s.data().readBy||[];if(!rb.includes(authUser.uid))updateDoc(convRef,{readBy:[...rb,authUser.uid]});}});
    return unsub;
  },[chatWith,authUser]);
  ...
  useEffect(()=>{
    ...
    return onSnapshot(q,snap=>{
      setComments(c=>({...c,[openComments]:snap.docs.map(d=>({id:d.id,...d.data()}))}));
    });
  },[openComments]);
 ...
 async function handleAuth(){
  ...
  async function toggleLike(postId,likes){
    ...
    await updateDoc(doc(db,"posts",postId),{likes:likes.includes(uid)?likes.filter(l=>l!==uid):[...likes,uid]});
  }
  ...
  return(
    <div style={S.root}>
      ...
      <div style={S.content}>
        ...
        {tab==="messages"&&!chatWith&&(<div>
          ...
          {conversations.map(conv=>{
            const otherId=conv.participants.find(p=>p!==authUser.uid);
            const otherName=conv.participantNames?.[otherId]||"Utilizator";
            const otherEmoji=conv.participantEmojis?.[otherId]||"🍺";
            const isUnread=conv.lastSenderId!==authUser.uid&&!(conv.readBy||[]).includes(authUser.uid);
            const otherUser=allUsers.find(u=>u.id===otherId);
            return(<div key={conv.id} style={{...S.postCard,cursor:"pointer",borderColor:isUnread?"#f5a623":"#242424"}} onClick={()=>otherUser&&setChatWith(otherUser)}>
              <div style={{display:"flex",gap:12,alignItems:"center"}}>
                <span style={{fontSize:32}}>{otherEmoji}</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:700,color:isUnread?"#f5a623":"#e8e0d0"}}>{otherName}</div>
                  <div style={{color:"#888",fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{conv.lastMessage}</div>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                  <span style={{color:"#555",fontSize:11}}>{timeAgo(conv.lastMessageAt)}</span>
                  {isUnread&&<span style={{background:"#f5a623",color:"#111",borderRadius:10,padding:"2px 7px",fontSize:11,fontWeight:700}}>nou</span>}
                </div>
              </div>
            </div>);
          })}
          <div style={{marginTop:20}}>
            <div style={{color:"#888",fontSize:13,marginBottom:10}}>Toți utilizatorii:</div>
            {allUsers.filter(u=>u.id!==authUser?.uid).map(u=>(<div key={u.id} style={{...S.nearbyCard,cursor:"pointer"}} onClick={()=>setChatWith(u)}>
        ...
        {tab==="messages"&&chatWith&&(<div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 140px)"}}>
          ...
          <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:8,paddingBottom:8}}>
            ...
            {messages.map(msg=>{const isMe=msg.senderId===authUser.uid;return(
              <div key={msg.id} style={{display:"flex",justifyContent:isMe?"flex-end":"flex-start"}}>
                <div style={{maxWidth:"75%",background:isMe?"linear-gradient(135deg,#f5a623,#e8890a)":"#1e1e1e",color:isMe?"#111":"#e8e0d0",borderRadius:isMe?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:"10px 14px",fontSize:14,lineHeight:1.5}}>
                  <div>{msg.text}</div>
                  <div style={{fontSize:10,opacity:0.6,marginTop:4,textAlign:"right"}}>{timeAgo(msg.createdAt)}</div>
                </div>
              </div>
            );})}
      ...
      <div style={S.nav}>
        {[{key:"feed",icon:"🏠",label:"Feed"},{key:"nearby",icon:"📍",label:"Aproape"},{key:"messages",icon:"💬",label:"Mesaje",badge:unreadCount},{key:"profile",icon:profile?.emoji,label:"Profil"}].map(t=>(
      ...
      {viewProfile&&(<div style={S.modal} onClick={()=>setViewProfile(null)}><div style={S.modalBox} onClick={e=>e.stopPropagation()}>
      ...
      {reviewTarget&&(<div style={S.modal} onClick={()=>setReviewTarget(null)}><div style={S.modalBox} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{fontSize:18,fontWeight:700,color:"#f5a623",marginBottom:16,textAlign:"center"}}>Recenzie pentru {reviewTarget.name}</div>
        <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:8}}>
          {[1,2,3,4,5].map(s=><button key={s} style={{background:"none",border:"none",cursor:"pointer",padding:2}} onMouseEnter={()=>setHoverRating(s)} onMouseLeave={()=>setHoverRating(0)} onClick={()=>setReviewRating(s)}><span style={{fontSize:28,color:s<=(hoverRating||reviewRating)?"#f5a623":"#444"}}>★</span></button>)}
        </div>
        <div style={{textAlign:"center",color:"#f5a623",marginBottom:12,fontSize:14}}>{["","Dezamăgire totală","Poate cu noroc","Ok, nimic special","Bun tovarăș","Legendă!"][hoverRating||reviewRating]}</div>
...
function ProfileView({user,posts,isOwn,onSignOut,onReview,onChat,styles:S,timeAgo,getTitle}){
  ...
  return(<div style={{paddingBottom:20}}>
    <div style={{textAlign:"center",paddingBottom:20,borderBottom:"1px solid #1e1e1e",marginBottom:16}}>
      ...
      {!isOwn&&<div style={{display:"flex",gap:8,marginTop:12}}>
    ...
    {(user.ratings||[]).length>0&&<div style={{marginBottom:16}}>
      <div style={{color:"#f5a623",fontSize:13,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Recenzii</div>
      {[...(user.ratings||[])].reverse().map((r,i)=>(<div key={i} style={{background:"#1a1a1a",borderRadius:10,padding:12,marginBottom:8}}>
    ...
    {userPosts.length>0&&<div>
      ...
      {userPosts.map(p=>(<div key={p.id} style={{background:"#1a1a1a",borderRadius:10,padding:12,marginBottom:8}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
